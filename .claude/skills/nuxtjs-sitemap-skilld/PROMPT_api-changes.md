...
Search: `skilld search "deprecated" -p @nuxtjs/sitemap`, `skilld search "breaking" -p @nuxtjs/sitemap`, `skilld search "v8.0" -p @nuxtjs/sitemap`, `skilld search "v7" -p @nuxtjs/sitemap`, `skilld search "Features" -p @nuxtjs/sitemap`

**Item scoring** — include only items scoring ≥ 3. Items scoring 0 MUST be excluded:

| Change type | v8.x | v7.x → v8.x migration | Older |
|-------------|:---:|:---:|:---:|
...
The "Older" column means ≤ v6.x — these changes are NOT useful because anyone on v8.x already migrated past them.
